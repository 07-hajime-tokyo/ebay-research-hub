...
import { getAllowedEmails, getSupabasePublicConfig, isEmailAllowed, isLocalAuthBypassEnabled } from "@/lib/env";
...
export async function isWorkspaceEmailAllowed(email?: string | null) {
  if (!email) return false;

  const allowed = getAllowedEmails();
  if (allowed.length > 0) return isEmailAllowed(email);
...
export async function requireAppUser() {
  ...
  if (!await isWorkspaceEmailAllowed(user.email)) redirect("/login?error=not_allowed");
  return user;
}
...
export async function requireRouteUser() {
  ...
  if (!await isWorkspaceEmailAllowed(user.email)) {
