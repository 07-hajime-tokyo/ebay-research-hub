...
function conversionRateFrom(sales, views, sourceValue) {
...
async function readGeneratedTrafficItems() {
  ...
  return items.map((item, index) => ({
    ...
    conversion_rate: Number(item.conversionRate || 0) || (Number(item.sold30 || item.sales || 0) > 0 && Number(item.views || 0) > 0 ? Number(item.sold30 || item.sales || 0) / Number(item.views || 0) : 0),
